...
export default function HeroSection() {
  ...
  const s = (key: string) => h?.[`${key}${lang === "en" ? "En" : "Zh"}`] || "";
    ...
    <section className="relative overflow-hidden decor-section">
      <div className="container-main relative z-10">
        <div className="grid items-center gap-12 py-16 md:min-h-[540px] md:grid-cols-[0.9fr_1.1fr] md:gap-12 md:py-16">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }} className="text-center md:text-left">
            <p className="eyebrow mb-4">{s("eyebrow")}</p>
            <h1 className="mb-6">{s("headline1")}<br />{s("headline2")}</h1>
            <p className="mb-8 max-w-md text-base leading-relaxed text-[var(--color-text-secondary)] md:text-lg">{s("description")}</p>
            <div className="flex flex-col items-center gap-4 sm:flex-row md:items-start">
              <Button href="/product/five-elements-blueprint">{s("cta")}</Button>
              <Button href="#sample" variant="outline">{s("sampleBtn")}</Button>
            </div>
            <div className="mt-10 flex items-center gap-4 text-xs text-[var(--color-text-secondary)] justify-center md:justify-start">
              <span>{s("trust1")}</span><span className="h-3 w-px bg-[var(--color-border)]" /><span>{s("trust2")}</span>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1], delay: 0.2 }} className="relative flex items-center justify-center min-h-[380px] md:min-h-[460px]">
