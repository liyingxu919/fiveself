...
function generateDayun(b:BaziResult):Array<{age:string;ganzhi:string;nayin:string;wuxing:string;desc:string;analysis:string}>{
  ...
  const yg=b.yearPillar.gan,dz=b.monthPillar.zhi;const fwd=yg%2===0;const sa=fwd?(12-dz+3)%10+3:(dz+8)%10+3;
  for(let i=0;i<8;i++){const a=sa+i*10;const zi=(dz+(fwd?i+1:-(i+1))+12)%12;const gi=(yg+(fwd?i+1:-(i+1))+10)%10;const gz=TIAN_GAN[gi]+DI_ZHI[zi];
    const na=NAYIN[gz]||"",wx=ZWX[zi];
    const anal=`${gz}大运，天干${TIAN_GAN[gi]}，地支${DI_ZHI[zi]}，纳音${na}。${na.includes("金")?"金运刚锐，利决断和执行，适合事业发展。":na.includes("木")?"木运生发，利学习和成长，适合积累知识和人脉。":na.includes("水")?"水运流动，利变通和智慧，适合转型和调整。":na.includes("火")?"火运光明，利名声和事业，适合积极进取。":na.includes("土")?"土运安稳，利积累和守成，适合巩固已有成果。":"五行中和，平稳过渡。"}${wx}气当令。`;
    r.push({age:`${a}岁`,ganzhi:gz,nayin:na,wuxing:wx,desc:["童蒙起步","学习成长","事业上升","人生鼎盛","稳健发展","收获成熟","守成传承","安享晚年"][i],analysis:anal});
  }return r;
}
